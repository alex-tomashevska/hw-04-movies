/** @format */

import { memo, useEffect, useState } from "react";
import axios from "axios";

import { useParams } from "react-router-dom";

import url from "../../services/baseUrl";
import imageApi from "../../services/baseUrl";
//import { getMovieCast } from "../../services";
import { transformationMoviesCast } from "../../utils";

import styles from "./Cast.module.css";

export const Cast = memo(() => {
  const [actors, setActors] = useState([]);
  const [isLoading, setLoading] = useState(false);

  const { id } = useParams();

  const getMovieCast = () => {
    setLoading(true);

    axios
      .get(url.movieCast(id))
      .then(({ data: { cast } }) => setActors(transformationMoviesCast(cast)))
      .catch(({ message }) => console.warn(message))
      .finally(() => setLoading(false));
  };

  //const handleActors = (results) => setActors(results);
  // const handleLoading = () => setLoading((prev) => !prev);

  useEffect(() => {
    getMovieCast();
  }, []);

  if (isLoading) {
    return <h1>Loading ...</h1>;
  }

  if (!actors.length) {
    return <p>There is no information about actors for this movie.</p>;
  }

  return (
    <div>
      <ul className={styles.list}>
        {actors.map(({ credit_id, profile_path, name, character }) => (
          <li key={credit_id} className={styles.item}>
            <img
              className={styles.img}
              src={imageApi.profilePath(profile_path)}
              alt={name}
            />
            <p>{name}</p>
            <p>Character: {character}</p>
          </li>
        ))}
      </ul>
    </div>
  );
});
